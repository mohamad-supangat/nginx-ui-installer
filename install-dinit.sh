...
# Service Path
ServicePath="/etc/dinit.d/nginx-ui"
...
identify_the_operating_system_and_architecture() {
    if [[ "$(uname)" == 'Linux' ]]; then
        ...
        *)
        ...
        ## Be aware of Linux distribution like Gentoo, which kernel supports switch between Dinit and OpenRC.
        
        if [[ -d /etc/dinit.d ]] || grep -q dinit <(ls -l /sbin/init); then
            true
        else
            echo -e "${FontRed}error: Only Linux distributions using dinit are supported.${FontSuffix}"
            exit 1
        fi
        if [[ "$(type -P apt)" ]]; then
...
cat > "$ServicePath" << EOF
type            = process
command         = /usr/local/bin/nginx-ui -config /usr/local/etc/nginx-ui/app.ini
smooth-recovery = true
waits-for       = loginready
EOF
    chmod 644 "$ServicePath"
    echo "info: dinit service files have been installed successfully!"
    ...
    service_cat_config "$ServicePath"
    dinitctl reload nginx-ui
    DINIT='1'
}
...
start_nginx_ui() {
    if [[ -f "$ServicePath" ]]; then
        dinitctl start nginx-ui
        sleep 1s
        if dinitctl is-started nginx-ui; then
...
stop_nginx_ui() {
    if ! dinitctl stop nginx-ui; then
...
remove_nginx_ui() {
  if dinitctl list | grep -qw 'nginx-ui'; then
    if [[ -n "$(pidof nginx-ui)" ]]; then
      stop_nginx_ui
    fi
    local delete_files=('/usr/local/bin/nginx-ui')
    if [[ "$PURGE" -eq '1' ]]; then
        [[ -d "$DataPath" ]] && delete_files+=("$DataPath")
    fi
    dinitctl disable nginx-ui
